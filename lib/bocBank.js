...
// Errors come back as RFC 7807 problem documents: code, title, detail and a
// requestId the bank asks you to quote in support requests.
function problem(data) {
  const code = data.code ? `${data.code} ` : "";
  const text = data.detail ?? data.title ?? JSON.stringify(data);
  const ref = data.requestId ? ` (requestId ${data.requestId})` : "";
  return `${code}${text}${ref}`;
}
...
export async function uploadAchFile(nachaContent, fileName) {
  ...
  if (!res.ok) {
    if (res.status === 409) {
      throw new Error(`DUPLICATE_FILE: ${problem(data)}`);
    }
    throw new Error(`BOC Bank error ${res.status}: ${problem(data)}`);
  }
...
export async function pollAchStatus(query) {
  ...
  if (!res.ok) {
    throw new Error(`BOC Bank status ${res.status}: ${problem(data)}`);
  }
