...
//
//   Upload — POST /api/v1/ach/files   (octet-stream body, X-API-Key auth)
//   Status — GET  /api/v1/ach/status  (X-API-Key auth)
//
...
export async function pollAchStatus(query) {
  ...
  if (query.fileId) qs.set("fileId", query.fileId);
  if (query.fileName) qs.set("fileName", query.fileName);
  if (query.fromDate) {
